...
class ServerSocket:
    def __init__(self, webhooks, printer):
        ...
        match(server_url.scheme):
            case 'tcp':
                try:
                    if server_url.netloc.startswith('['):# IPv6 address
                    ...
                    logging.exception("webhooks: Unable to bind to '%s:%d'"% (server_url.hostname, server_url.port))
            case _:
                try:
                    ...
                    logging.exception("webhooks: Unable to bind to unix socket '%s'"% (server_url.path))
                    return
        self.sock.listen(1)
...
class GCodeHelper:
    def __init__(self, printer):
        ...
        wh.register_endpoint("gcode/session", self._handle_session)
    ...
    def _handle_session(self, web_request):
        gcode = web_request.get_str('command')
        outputs = []
        def output_handler(message):
            outputs.append(message)
        self.gcode.register_output_handler(output_handler)
        try:
            self.gcode.run_command(gcode)
            web_request.send({"output": outputs})
        finally:
            self.gcode.remove_output_handler(output_handler)
