...
#homing_center:
#   If true, homing will cause the stepper to move in a positive
#   direction if endstop is in triggered state in current position
#   and in negative direction if it's open,
#   in either case will seek position where state of endstop changes.
#   This option is mutually exclusive with homing_positive_dir
#   seting both will cause config error
```
...
#brightness: 255
#   Sets max brightness of a strip.
#   this is multiplier by wich 0.-1. range is scaled before being sent
#   out to led strip. Let you keep config of animations intact.
#   lower values make strip dimmer but also affect color resolution.
#initial_RED: 0.0
#initial_GREEN: 0.0
#initial_BLUE: 0.0
#initial_WHITE: 0.0
